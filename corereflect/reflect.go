...
func InvokeFuncReturnError(objRef interface{}, nfunc string, args ...interface{}) error {
	inputs := make([]reflect.Value, len(args))
	for i, v := range args {
		inputs[i] = reflect.ValueOf(v)
	}
	res := reflect.ValueOf(objRef).MethodByName(nfunc).Call(inputs)
	if len(res) > 0 {
		e := res[0].Interface()
		if e != nil {
			return e.(error)
...
//Ejecuta metodo por refleccion
func InvokeFuncReturnValueAndError(objRef interface{}, nfunc string, args ...interface{}) (interface{}, error) {
	inputs := make([]reflect.Value, len(args))
	for i, v := range args {
		inputs[i] = reflect.ValueOf(v)
	}
